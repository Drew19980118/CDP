...
# ---------------------------
# 2) Soft Reordering Mechanism (论文核心)
# ---------------------------
class SoftReorderingMechanism(nn.Module):
    def __init__(self, input_dim=156, hidden_dim=1024, output_channels=64, feature_width=16):
        super(SoftReorderingMechanism, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_channels = output_channels
        self.feature_width = feature_width

        # 全连接层 + 非线性激活 (论文中使用ReLU)
        self.fc_layer = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, output_channels * feature_width),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        transformed = self.fc_layer(x_reshaped)

        # 重塑为多通道形式 [batch_size, seq_len, output_channels, feature_width]
        transformed = transformed.reshape(batch_size, seq_len, self.output_channels, self.feature_width)

        # 调整维度顺序为 [batch_size, output_channels, seq_len, feature_width]
        # 但1D CNN期望 [batch_size, channels, sequence_length]
        # 我们需要将seq_len和feature_width合并
        transformed = transformed.permute(0, 2, 1, 3).contiguous()
        transformed = transformed.view(batch_size, self.output_channels, -1)

        return transformed


# ---------------------------
# 3) 1D CNN Architecture (论文架构)
# ---------------------------
class SR1DCNN(nn.Module):
    def __init__(self, input_dim=156, num_classes=5, hidden_dim=1024,
                 output_channels=64, feature_width=16):
        super(SR1DCNN, self).__init__()

        # Soft Reordering Mechanism
        self.soft_reordering = SoftReorderingMechanism(
            input_dim=input_dim,
        ...
        self.conv_layers = nn.Sequential(
            # Conv1: kernel_size=7, channels: 64 -> 128
            nn.Conv1d(output_channels, 128, kernel_size=7, padding=3),
            nn.BatchNorm1d(128),
            nn.ReLU(),
    ...
    def forward(self, batch_data):
        ...
        # Soft Reordering
        reordered_features = self.soft_reordering(financial_data)

        # 1D CNN
        cnn_features = self.conv_layers(reordered_features)

        # 全局平均池化 + 展平
        cnn_features = F.adaptive_avg_pool1d(cnn_features, 16)
...
class ComprehensiveTester:
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        with open(txt_filename, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Model: Soft Reordering 1D-CNN (SR-1D-CNN)\n")
            f.write(f"Input dimensions: Financial Panel (156 features)\n")
            f.write(f"Soft Reordering hidden dimension: 1024\n")
            f.write(f"Output channels: 64\n")
            f.write(f"Feature width: 16\n")
            f.write(f"CNN layers: 4 convolutional layers\n")
...
def main():
    ...
    print("Starting Soft Reordering 1D-CNN Comprehensive Training...")
