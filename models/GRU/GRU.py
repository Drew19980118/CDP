...
# Assuming your dataset class is in data_loader.py
try:
    from data_loader import YearlyFinancialDataset
except ImportError:
    print("Warning: Could not import YearlyFinancialDataset, make sure data_loader.py is accessible")
...
# -----------------------------------
# 2) GRU Model for Financial Data Only
# -----------------------------------
class FinancialDataProcessor(nn.Module):
    def __init__(self, financial_dim=26, output_dim=128, hidden_dim=256, dropout=0.05):
...
class GRUFinancialPredictor(nn.Module):
    """GRU model for financial default prediction using only financial panel data"""

    def __init__(self, input_dim=26, hidden_dim=128, num_layers=2,
                 dropout=0.2, num_classes=2, bidirectional=False):
        super(GRUFinancialPredictor, self).__init__()
        ...
        self.classifier = nn.Sequential(
            ...
            nn.LayerNorm(gru_output_dim // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(gru_output_dim // 2, gru_output_dim // 4),
            nn.LayerNorm(gru_output_dim // 4),
            nn.ReLU(),
            nn.Dropout(max(0.01, dropout / 2)),
            nn.Linear(gru_output_dim // 4, num_classes)
        )
    ...
    def forward(self, batch_data):
        # Extract financial data - assuming shape [batch_size, seq_len, features]
        features = batch_data['features']

        # Process financial data through MLP
        financial_processed = self.financial_processor(features)

        # Pass through GRU
        gru_out, hidden = self.gru(financial_processed)
...
def collate_fn(batch):
    """Custom collate function for financial data"""
    features = []
    targets = []
...
# ---------------------------
class GRUTrainer:
    """Comprehensive GRU trainer with detailed metrics tracking"""
    ...
    def __init__(self, model, train_loader, val_loader, device, learning_rate=3e-4, loss_type='crossentropy'):
        ...
        # Optimizer and loss function - matching your original code
    ...
    def calculate_metrics(self, targets, predictions, probabilities=None):
        ...
        if probabilities is not None and len(np.unique(targets)) > 1:
            try:
                auc = roc_auc_score(targets, probabilities[:, 1] if probabilities.shape[1] > 1 else probabilities[:, 0])
            except:
                auc = 0.0
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            # Print batch-level info occasionally
            if (batch_idx + 1) % 100 == 0:
                ...
                print(
                    f"  Batch {batch_idx + 1}/{len(self.train_loader)} - loss: {loss.item():.4f} - grad_norm: {total_grad_norm:.4f}")
    ...
    def validate(self, epoch):
        ...
        accuracy, precision, recall, f1, auc = self.calculate_metrics(
            all_targets, all_predictions, all_probabilities_array
        )
    ...
    def find_best_epoch(self):
        """Find the best epoch based on comprehensive performance across all metrics"""
        print(f"\n{'=' * 60}")
        ...
        for epoch in range(len(self.val_accuracies)):
            # Comprehensive score considering all important metrics
            # We want balanced performance across accuracy, precision, recall, F1, and AUC
            comprehensive_score = (
                                          self.val_accuracies[epoch] +
                                          self.val_precisions[epoch] +
                                          self.val_recalls[epoch] +
                                          self.val_f1_scores[epoch] +
                                          self.val_auc_scores[epoch]
                                  ) / 5.0
            ...
            print(f"Epoch {epoch + 1}: Comprehensive Score = {comprehensive_score:.4f} "
                  ...
                  f"AUC: {self.val_auc_scores[epoch]:.4f})")
        ...
        if best_epoch > 0:
            ...
            print(f"AUC: {self.val_auc_scores[best_epoch - 1]:.4f}")
    ...
    def train(self, num_epochs=250, save_path=None):
        ...
        # Find best epoch based on comprehensive metrics
        best_epoch = self.find_best_epoch()

        # Save best model
        if save_path:
            self.save_best_model(save_path, best_epoch)
            self.plot_training_metrics(save_path)
    ...
    def plot_training_metrics(self, save_path):
        ...
        # Loss
        ax1.plot(epochs, self.train_losses, linewidth=2, label='Training Loss')
        ax1.plot(epochs, self.val_losses, linewidth=2, label='Validation Loss')
        ...
        # Accuracy
        ax2.plot(epochs, self.train_accuracies, linewidth=2, label='Training Accuracy')
        ax2.plot(epochs, self.val_accuracies, linewidth=2, label='Validation Accuracy')
        if self.best_epoch > 0:
        ...
        # F1-Score and AUC
        ax3.plot(epochs, self.val_f1_scores, linewidth=2, label='Validation F1-Score')
        ax3.plot(epochs, self.val_auc_scores, linewidth=2, label='Validation AUC')
        if self.best_epoch > 0:
            ax3.axvline(x=self.best_epoch, color='r', linestyle='--',
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.7)
        ax3.set_title('Validation F1-Score and AUC', fontsize=14, fontweight='bold')
        ...
        ax4.plot(epochs, self.val_precisions, linewidth=2, label='Validation Precision')
        ax4.plot(epochs, self.val_recalls, linewidth=2, label='Validation Recall')
        if self.best_epoch > 0:
            ax4.axvline(x=self.best_epoch, color='r', linestyle='--',
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.7)
        ax4.set_title('Validation Precision and Recall', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}_training_metrics.png', dpi=300, bbox_inches='tight')
        plt.show()
...
class GRUTester:
    ...
    def calculate_detailed_metrics(self, targets, predictions, probabilities, num_classes):
    ...
    def test(self, model_path=None, best_epoch=None, avg_epoch_time=None):
        ...
        # Calculate comprehensive metrics
        metrics = self.calculate_detailed_metrics(all_targets, all_predictions, all_probabilities_array, num_classes)
        ...
        self.save_comprehensive_results(
            test_loss, metrics, all_targets, all_predictions, all_probabilities_array,
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        txt_filename = f'gru_financial_test_results_{timestamp}.txt'

        with open(txt_filename, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("            GRU FINANCIAL PREDICTOR - TEST RESULTS REPORT\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write(f"AUC: {metrics['overall']['auc']:.6f}\n\n")
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Model: GRU Financial Predictor\n")
            f.write(f"Input dimension: {self.model.input_dim}\n")
            f.write(f"Hidden dimension: {self.model.hidden_dim}\n")
            f.write(f"GRU layers: {self.model.num_layers}\n")
            f.write(f"Bidirectional: {self.model.bidirectional}\n")
            f.write(f"Number of classes: {self.model.num_classes}\n")
...
def load_datasets():
    ...
    if len(train_dataset) > 0:
        ...
        feature_dim = 26  # default fallback

    return train_dataset, val_dataset, test_dataset, num_classes, feature_dim


def main():
    """Main training function"""
    print("Starting GRU Financial Predictor Training...")
    ...
    # Load data
    train_dataset, val_dataset, test_dataset, num_classes, feature_dim = load_datasets()

    # Create model - adjusted for financial data only
    model = GRUFinancialPredictor(
    ...
    # Create comprehensive trainer with same learning rate and loss as your original code
    trainer = GRUTrainer(
        model=model,
        ...
        device=device,
        learning_rate=3e-4,
        loss_type='crossentropy'  # Using crossentropy as in your original code
    )

    # Create save directory
    os.makedirs('..', exist_ok=True)
    save_path = '../../../models/GRU/gru'
    ...
    tester = GRUTester(model, test_loader, device)
