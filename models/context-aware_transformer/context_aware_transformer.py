...
# -----------------------------------
# 2) Processors (BatchNorm -> LayerNorm)
# -----------------------------------
class FinancialDataProcessor(nn.Module):
    def __init__(self, financial_dim=156, output_dim=256, hidden_dim=512, dropout=0.05):
        ...
        # Use LayerNorm instead of BatchNorm1d for small batch / sequence stability
...
class IndustryEmbedding(nn.Module):
    def __init__(self, num_industries=5, output_dim=256):
...
class InputRepresentation(nn.Module):
    def __init__(self, financial_dim=156, macro_dim=36, num_industries=5,
                 financial_hidden=512, macro_hidden=256, output_dim=256):
    ...
    def forward(self, batch_data):
        ...
        industry_ids = batch_data['industry']

        batch_size, seq_len, total_features = features.shape

        # Separate features
        financial_data = features[:, :, :156]
        macro_data = features[:, :, 156:156 + 36]
        ...
        # Combine financial + macro features (element-wise add is simple and effective)
        quarterly_combined = financial_embedding + macro_embedding

        # Add positional encoding (only for quarterly data)
        quarterly_with_pos = self.positional_encoding(quarterly_combined)

        # Process industry information
        industry_embedding = self.industry_embedding(industry_ids)

        # Expand industry dimension: [batch_size, 256] -> [batch_size, 1, 256]
        industry_embedding_expanded = industry_embedding.unsqueeze(1)

        # Industry info at first position, followed by 4 quarters
        final_representation = torch.cat([industry_embedding_expanded, quarterly_with_pos], dim=1)
...
class CausalAttentionMask:
    ...
    def generate_mask(seq_len, device):
        ...
        mask[0, :] = 0  # Industry can see all
        for i in range(1, seq_len):
            mask[i, :i + 1] = 0  # Quarters can only see industry and previous quarters
        return mask
...
class ContextAwareTransformerEncoder(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_layers=4,
        ...
        self.input_representation = InputRepresentation(output_dim=d_model)
        ...
        # Output classifier (use LayerNorm in MLP blocks instead of BatchNorm)
    ...
    def forward(self, batch_data):
        ...
        embeddings = self.input_representation(batch_data)
        batch_size, seq_len, d_model = embeddings.shape

        # 2. Generate causal attention mask
        attention_mask = CausalAttentionMask.generate_mask(seq_len, embeddings.device)
        ...
        # 4. Extract industry representation for classification
        industry_representation = x[:, 0, :]
        logits = self.classifier(industry_representation)
...
class ComprehensiveTrainer:
    ...
    def __init__(self, model, train_loader, val_loader, device, learning_rate=3e-4, loss_type='focal'):
        ...
        # Learning rate scheduler (epoch-based). You can experiment with batch-based schedulers later.
    ...
    def plot_validation_metrics(self, save_path):
        """Plot validation metrics across epochs - SEPARATE PDFs with different colors"""
        ...
        plt.plot(epochs, self.val_accuracies, linewidth=2.5, color=colors['accuracy'], label='Validation Accuracy')
        if self.best_epoch > 0:
            plt.axvline(x=self.best_epoch, color='black', linestyle='--', linewidth=2,
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.8)
        plt.title('Validation Accuracy Across Epochs', fontsize=16, fontweight='bold')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Accuracy', fontsize=12)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        accuracy_pdf_path = f'{save_path}_validation_accuracy.pdf'
        plt.savefig(accuracy_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
        plt.close()
        print(f"📊 Accuracy plot saved to: {accuracy_pdf_path}")

        # 2. Precision Plot
        plt.figure(figsize=(10, 6))
        plt.plot(epochs, self.val_precisions, linewidth=2.5, color=colors['precision'], label='Validation Precision')
        if self.best_epoch > 0:
            plt.axvline(x=self.best_epoch, color='black', linestyle='--', linewidth=2,
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.8)
        plt.title('Validation Precision Across Epochs', fontsize=16, fontweight='bold')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Precision', fontsize=12)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        precision_pdf_path = f'{save_path}_validation_precision.pdf'
        plt.savefig(precision_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
        plt.close()
        print(f"📊 Precision plot saved to: {precision_pdf_path}")

        # 3. Recall Plot
        plt.figure(figsize=(10, 6))
        plt.plot(epochs, self.val_recalls, linewidth=2.5, color=colors['recall'], label='Validation Recall')
        if self.best_epoch > 0:
            plt.axvline(x=self.best_epoch, color='black', linestyle='--', linewidth=2,
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.8)
        plt.title('Validation Recall Across Epochs', fontsize=16, fontweight='bold')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Recall', fontsize=12)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        recall_pdf_path = f'{save_path}_validation_recall.pdf'
        plt.savefig(recall_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
        plt.close()
        print(f"📊 Recall plot saved to: {recall_pdf_path}")

        # 4. F1-Score Plot
        plt.figure(figsize=(10, 6))
        plt.plot(epochs, self.val_f1_scores, linewidth=2.5, color=colors['f1'], label='Validation F1-Score')
        if self.best_epoch > 0:
            plt.axvline(x=self.best_epoch, color='black', linestyle='--', linewidth=2,
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.8)
        plt.title('Validation F1-Score Across Epochs', fontsize=16, fontweight='bold')
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('F1-Score', fontsize=12)
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        f1_pdf_path = f'{save_path}_validation_f1_score.pdf'
        plt.savefig(f1_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
        plt.close()
        print(f"📊 F1-Score plot saved to: {f1_pdf_path}")

        # 5. Combined metrics plot (optional, for reference)
        plt.figure(figsize=(12, 8))
        plt.plot(epochs, self.val_accuracies, linewidth=2.5, color=colors['accuracy'], label='Accuracy')
        plt.plot(epochs, self.val_precisions, linewidth=2.5, color=colors['precision'], label='Precision')
        plt.plot(epochs, self.val_recalls, linewidth=2.5, color=colors['recall'], label='Recall')
        plt.plot(epochs, self.val_f1_scores, linewidth=2.5, color=colors['f1'], label='F1-Score')
        if self.best_epoch > 0:
            plt.axvline(x=self.best_epoch, color='black', linestyle='--', linewidth=2,
                        label=f'Best Epoch: {self.best_epoch}', alpha=0.8)
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        combined_pdf_path = f'{save_path}_comprehensive_metrics.pdf'
        plt.savefig(combined_pdf_path, dpi=300, bbox_inches='tight', format='pdf')
        plt.close()
        print(f"📊 Combined metrics plot saved to: {combined_pdf_path}")


# ---------------------------
# Tester (unchanged except small polish)
# ---------------------------
class ComprehensiveTester:
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        with open(txt_filename, 'w', encoding='utf-8') as f:
            ...
            # Model info: try to read from model attribute if present
            f.write("-" * 50 + "\n")
            f.write("MODEL INFORMATION\n")
            f.write("-" * 50 + "\n")
            try:
                layers = getattr(self, 'model').num_layers
            except Exception:
                layers = 'Unknown'
            f.write(f"Model: Context-aware Transformer\n")
            f.write(f"Input dimensions: Financial (156) + Macro (36)\n")
            f.write(f"Transformer layers: {layers}\n")
...
def sanity_check_overfit(train_dataset, device, num_classes):
    """Small helper: build tiny model and try to overfit small subset (sanity check)"""
    print("\nRunning sanity check overfit on 100 samples (or fewer if dataset smaller)...")
    small_n = min(100, len(train_dataset))
    small_subset = train_dataset[:small_n]
    small_loader = DataLoader(small_subset, batch_size=8, shuffle=True, collate_fn=collate_fn)

    tiny_model = ContextAwareTransformerEncoder(
        d_model=256,
        nhead=8,
        num_layers=1,
        dim_feedforward=512,
    ...
    tiny_trainer = ComprehensiveTrainer(
        model=tiny_model,
    ...
    best_epoch, avg_time = tiny_trainer.train(num_epochs=80, save_path=None)
    print("Sanity check finished.\n")


def main():
    """Main training function"""
    print("Starting Context-aware Transformer Comprehensive Training...")
    ...
    # Optional sanity check (uncomment to run quick overfit test)
    # sanity_check_overfit(train_dataset, device, num_classes)

    # Create model
    model = ContextAwareTransformerEncoder(
        d_model=256,
        nhead=8,
        num_layers=4,         # reduced default for easier convergence
    ...
    # Create comprehensive trainer (defaults changed: lr=3e-4)
    trainer = ComprehensiveTrainer(
        ...
        device=device,
        learning_rate=3e-4,
        loss_type='crossentropy'  # set to 'focal' to try focal loss (gamma=1)
    )

    # Create save directory
    os.makedirs('..', exist_ok=True)
    save_path = '../../../models/context-aware_transformer/context_aware_transformer'

    # Start training (you can try small num_epochs first like 20 to inspect behavior)
