...
# ---------------------------
# Financial Dataset (Only Financial Panel)
# ---------------------------
class FinancialPanelDataset(Dataset):
    """Dataset for financial panel data only (no macro/industry info)"""

    def __init__(self, features, targets):
        """
        Args:
            features: numpy array of shape (num_samples, 4, financial_dim) - 4 quarters
            targets: numpy array of shape (num_samples,) - single year prediction
        """
...
# ---------------------------
# ANN Model (Financial Panel Only)
# ---------------------------
class FinancialANN(nn.Module):
    """ANN model for financial panel data prediction"""

    def __init__(self, financial_dim=156, hidden_dims=[512, 256, 128], num_classes=5, dropout_rate=0.2):
        super(FinancialANN, self).__init__()

        self.financial_dim = financial_dim
        self.num_classes = num_classes

        # Flatten quarterly financial data: (batch, 4, financial_dim) -> (batch, 4*financial_dim)
        input_dim = 4 * financial_dim
    ...
    def forward(self, x):
        # x shape: (batch_size, 4, financial_dim)
        batch_size = x.shape[0]

        # Flatten quarterly data
        x_flat = x.reshape(batch_size, -1)  # (batch_size, 4*financial_dim)
...
# ---------------------------
class FinancialANNTrainer:
    """Trainer for Financial ANN model"""
...
# ---------------------------
class FinancialANNTester:
    """Comprehensive tester for Financial ANN model"""
    ...
    def plot_confusion_matrix(self, targets, predictions):
        ...
        plt.title('Confusion Matrix - Financial ANN', fontsize=16, fontweight='bold')
        plt.ylabel('True Label', fontsize=12)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.tight_layout()
        plt.savefig('confusion_matrix_financial_ann.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        txt_filename = f'financial_ann_test_results_{timestamp}.txt'

        with open(txt_filename, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("            FINANCIAL ANN COMPREHENSIVE TEST RESULTS\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Model: Financial ANN (MLP)\n")
            f.write(f"Input dimensions: Financial Panel (156 features × 4 quarters = 624)\n")
            f.write(f"Hidden layers: [512, 256, 128]\n")
...
# ---------------------------
def load_financial_datasets(data_path):
    """Load financial datasets from pickle files"""
    print("Loading financial datasets...")

    # Load training set
    with open(os.path.join(data_path, 'train_set/train_dataset_split1.pkl'), 'rb') as f:
        train_data = pickle.load(f)

    # Load validation set
    with open(os.path.join(data_path, 'validation_set/validation_dataset_split1.pkl'), 'rb') as f:
        val_data = pickle.load(f)

    # Load test set
    with open(os.path.join(data_path, 'test_set/test_dataset_split1.pkl'), 'rb') as f:
        test_data = pickle.load(f)

    # Extract features and targets (only financial data)
    def extract_financial_data(dataset):
        features = []
        targets = []

        for sample in dataset:
            # Extract only financial features (first 156 dimensions)
            financial_features = sample['features'][:, :156]  # (4, 156)
            features.append(financial_features)
            targets.append(sample['target'])

        return np.array(features), np.array(targets)

    train_features, train_targets = extract_financial_data(train_data)
    val_features, val_targets = extract_financial_data(val_data)
    test_features, test_targets = extract_financial_data(test_data)
    ...
    print(f"Feature shape: {train_features[0].shape}")
    ...
    # Create datasets
    train_dataset = FinancialPanelDataset(train_features, train_targets)
    val_dataset = FinancialPanelDataset(val_features, val_targets)
    test_dataset = FinancialPanelDataset(test_features, test_targets)
...
def main():
    ...
    print("Starting Financial ANN Training and Testing...")
    ...
    train_dataset, val_dataset, test_dataset, num_classes = load_financial_datasets(data_path)
    ...
    trainer = FinancialANNTrainer(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        device=device,
        learning_rate=3e-4,
        loss_type='crossentropy'  # Using cross entropy as in your original code
    )

    # Create save directory
    save_path = '../../../models/ANN/financial_ann_model'
    ...
    tester = FinancialANNTester(model, test_loader, device)
