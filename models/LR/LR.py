# logistic_regression_financial_macro.py
import torch
...
# ---------------------------
# 2) Logistic Regression Model (Financial + Macro Data)
# ---------------------------
class FinancialMacroLogisticRegression(nn.Module):
    """Logistic Regression model for Financial Panel + Macro data"""

    def __init__(self, input_dim=768, num_classes=5, dropout=0.1):
        super(FinancialMacroLogisticRegression, self).__init__()

        # 输入维度: (156 financial + 36 macro) * 4 quarters = 768
    ...
    def forward(self, batch_data):
        ...
        # 提取金融面板数据 (前156个特征) 和宏观数据 (接下来的36个特征)
        # 形状: [batch_size, seq_len=4, total_features=192]
        batch_size, seq_len, total_features = features.shape

        # 只使用金融和宏观数据，忽略行业信息
        financial_macro_data = features[:, :, :156 + 36]  # [batch_size, 4, 192]

        # 展平序列维度: [batch_size, 4 * 192 = 768]
        flattened_features = financial_macro_data.reshape(batch_size, -1)
...
class ComprehensiveTester:
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        with open(txt_filename, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Model: Financial + Macro Logistic Regression\n")
            f.write(f"Input dimensions: Financial Panel (156) + Macro (36) * 4 quarters = 768\n")
            f.write(f"Hidden layers: 512 -> 256 -> 128\n")
            f.write(f"Activation: ReLU\n")
            f.write(f"Normalization: LayerNorm\n")
...
def main():
    ...
    print("Starting Financial + Macro Logistic Regression Comprehensive Training...")
    ...
    # Create model - Financial + Macro Logistic Regression
    model = FinancialMacroLogisticRegression(
        input_dim=768,  # (156 financial + 36 macro) * 4 quarters
