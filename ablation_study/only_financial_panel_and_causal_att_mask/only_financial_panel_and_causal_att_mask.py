# ablation_baseline_financial_only.py
import torch
...
# -----------------------------------
# 2) Processors for Financial Data Only
# -----------------------------------
...
class InputRepresentationBaseline(nn.Module):
    """Input representation for baseline: only financial data"""

    def __init__(self, financial_dim=156, output_dim=256, financial_hidden=512):
        super(InputRepresentationBaseline, self).__init__()
    ...
    def forward(self, batch_data):
        ...
        # Extract only financial data (first 156 features)
...
class CausalAttentionMaskBaseline:
    """Causal attention mask for baseline (only quarterly data)"""

    @staticmethod
    def generate_mask(seq_len, device):
        # For baseline: only 4 quarterly tokens, no industry token
...
class BaselineFinancialTransformer(nn.Module):
    """Baseline model: Only financial data with causal attention"""
    ...
    def __init__(self, d_model=256, nhead=8, num_layers=4,
        ...
        self.seq_len = 4  # Only 4 quarters, no industry token

        # Input representation layer (financial data only)
        self.input_representation = InputRepresentationBaseline(output_dim=d_model)
    ...
    def forward(self, batch_data):
        # 1. Input representation (financial data only)
        embeddings = self.input_representation(batch_data)
        batch_size, seq_len, d_model = embeddings.shape

        # 2. Generate causal attention mask (for 4 quarters only)
        attention_mask = CausalAttentionMaskBaseline.generate_mask(seq_len, embeddings.device)
        ...
        last_quarter_representation = x[:, -1, :]  # Use last quarter
...
class AblationTrainer:
    ...
    def save_best_model(self, save_path, best_epoch):
        ...
        print(f"💾 Best baseline model saved to: {save_path}_best_epoch_{best_epoch}.pth")
...
class AblationTester:
    ...
    def plot_confusion_matrix(self, targets, predictions):
        ...
        plt.title('Confusion Matrix - Baseline (Financial Only)', fontsize=16, fontweight='bold')
        plt.ylabel('True Label', fontsize=12)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.tight_layout()
        plt.savefig('confusion_matrix_baseline_financial_only.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def save_comprehensive_results(self, test_loss, metrics, targets, predictions, probabilities,
        ...
        txt_filename = f'performance_baseline_financial_only_{timestamp}.txt'

        with open(txt_filename, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("    ABLATION STUDY: BASELINE (FINANCIAL DATA ONLY) - PERFORMANCE REPORT\n")
            f.write("=" * 70 + "\n\n")

            f.write(f"Test time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model: BaselineFinancialTransformer\n")
            f.write(f"Input: Financial data only (156 features)\n")
            f.write(f"Attention: Causal mask\n")
            ...
            f.write("-" * 50 + "\n")
            f.write(f"Model: BaselineFinancialTransformer\n")
            f.write(f"Input dimensions: Financial (156 features)\n")
            ...
            f.write(f"Attention mask: Causal (no industry token)\n\n")
...
def main():
    """Main function for baseline ablation study"""
    print("Starting Baseline Ablation Study: Financial Data Only")
    print("Model: BaselineFinancialTransformer (Financial Only + Causal Mask)")
    ...
    model = BaselineFinancialTransformer(
        d_model=256,
        nhead=8,
        num_layers=4,
    ...
    print(f"\nBaseline Model parameter statistics:")
    print(f"  Total parameters: {total_params:,}")
    print(f"  Input: Financial data only (156 features)")
    print(f"  Sequence length: 4 quarters (no industry token)")
    print(f"  Attention: Causal mask only")
    ...
    os.makedirs('baseline_financial_only_and_causal_att_mask', exist_ok=True)
    save_path = 'baseline_financial_only_and_causal_att_mask'

    # Start training
    best_epoch, avg_epoch_time = trainer.train(num_epochs=250, save_path=save_path)

    print(f"\nBaseline ablation training completed! Best epoch: {best_epoch}")
    if avg_epoch_time is not None:
        print(f"Average training time per epoch: {avg_epoch_time:.2f} seconds")
    print(f"Best baseline model saved to: {save_path}_best_epoch_{best_epoch}.pth")
    ...
    print(f"\n🎯 Baseline ablation study completed successfully!")
    print(f"📊 Performance report saved as: performance_baseline_financial_only_*.txt")
    print(f"📈 Confusion matrix saved as: confusion_matrix_baseline_financial_only.png")
