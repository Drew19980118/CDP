...
# -----------------------------------
# 2) Processors for Financial + Industry Data
# -----------------------------------
...
class InputRepresentationIndustry(nn.Module):
    """Input representation for baseline + industry"""

    def __init__(self, financial_dim=156, num_industries=5, output_dim=256, financial_hidden=512):
        super(InputRepresentationIndustry, self).__init__()
    ...
    def forward(self, batch_data):
        ...
        # Extract only financial data (first 156 features) - NO macroeconomic data
        financial_data = features[:, :, :156]

        # Process financial data only
        financial_embedding = self.financial_processor(financial_data)

        # Add positional encoding to financial data
        quarterly_with_pos = self.positional_encoding(financial_embedding)
...
class StructuredAttentionMaskIndustry:
    """Structured attention mask for industry model (industry token + 4 quarters)"""

    @staticmethod
    def generate_mask(seq_len, device):
        # For industry model: 1 industry token + 4 quarterly tokens
        # Mask shape: (5, 5) with -inf for masked positions
        mask = torch.full((seq_len, seq_len), float('-inf'), device=device)

        # Industry token (index 0): can see all tokens
        mask[0, :] = 0

        # Quarterly tokens (indices 1-4):
        # Each quarter can see industry token and itself + previous quarters
        for i in range(1, seq_len):  # i = 1,2,3,4
            mask[i, 0] = 0  # Can see industry token
            mask[i, 1:i + 1] = 0  # Can see itself and previous quarters
...
class IndustryAwareTransformer(nn.Module):
    """Industry-aware model: Financial data + Industry with structured attention"""
    ...
    def __init__(self, d_model=256, nhead=8, num_layers=4,
        ...
        # Input representation layer (financial data + industry)
        self.input_representation = InputRepresentationIndustry(
            financial_dim=156,
            num_industries=num_industries,
            output_dim=d_model
        )
    ...
    def forward(self, batch_data):
        # 1. Input representation (financial data + industry)
        embeddings = self.input_representation(batch_data)
        batch_size, seq_len, d_model = embeddings.shape

        # 2. Generate structured attention mask (industry + 4 quarters)
        attention_mask = StructuredAttentionMaskIndustry.generate_mask(seq_len, embeddings.device)
...
class AblationTrainer:
    ...
    def save_best_model(self, save_path, best_epoch):
        ...
        print(f"💾 Best industry model saved to: {save_path}_best_epoch_{best_epoch}.pth")
...
class AblationTester:
    ...
    def plot_confusion_matrix(self, targets, predictions):
        ...
        plt.title('Confusion Matrix - Industry Model', fontsize=16, fontweight='bold')
        plt.ylabel('True Label', fontsize=12)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.tight_layout()
        plt.savefig('confusion_matrix_industry_model.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    """Main function for industry ablation study"""
    print("Starting Industry Ablation Study: Financial Data + Industry")
    print("Model: IndustryAwareTransformer (Financial + Industry + Structured Mask)")
    ...
    # Create industry-aware model
    model = IndustryAwareTransformer(
        d_model=256,
        nhead=8,
        num_layers=4,
    ...
    print(f"\nIndustry Model parameter statistics:")
    print(f"  Total parameters: {total_params:,}")
    print(f"  Input: Financial data (156 features) + Industry information")
    print(f"  Sequence length: 5 (industry token + 4 quarters)")
    print(f"  Attention: Structured mask")
    ...
    os.makedirs('industry_model', exist_ok=True)
    save_path = 'industry_model'

    # Start training
    best_epoch, avg_epoch_time = trainer.train(num_epochs=250, save_path=save_path)

    print(f"\nIndustry ablation training completed! Best epoch: {best_epoch}")
    if avg_epoch_time is not None:
        print(f"Average training time per epoch: {avg_epoch_time:.2f} seconds")
    print(f"Best industry model saved to: {save_path}_best_epoch_{best_epoch}.pth")
    ...
    print(f"\n🎯 Industry ablation study completed successfully!")
    print(f"📊 Performance report saved as: performance_industry_model_*.txt")
    print(f"📈 Confusion matrix saved as: confusion_matrix_industry_model.png")
