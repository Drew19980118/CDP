# ablation_full_model_no_structured_mask.py
import torch
...
# -----------------------------------
# 2) Processors for Full Model (Financial + Macro + Industry)
# -----------------------------------
...
class InputRepresentationFullModel(nn.Module):
    """Input representation for full model: financial + macro + industry"""

    def __init__(self, financial_dim=156, macro_dim=36, num_industries=5,
                 financial_hidden=512, macro_hidden=256, output_dim=256):
        super(InputRepresentationFullModel, self).__init__()
    ...
    def forward(self, batch_data):
        ...
        # Combine financial + macro features (element-wise add)
        ...
        return final_representation  # Shape: [batch_size, 5, 256]


class FullConnectionAttentionMask:
    """Full connection attention mask (no restrictions)"""

    @staticmethod
    def generate_mask(seq_len, device):
        # Full connection: all tokens can see all other tokens
        # Mask shape: (seq_len, seq_len) with all zeros (no masking)
...
class FullModelNoStructuredMask(nn.Module):
    """Full model without structured mask: Financial + Macro + Industry with full connection"""

    def __init__(self, d_model=256, nhead=8, num_layers=4,
                 dim_feedforward=1024, dropout=0.05, num_classes=5, num_industries=5):
        super().__init__()
        self.d_model = d_model
        self.num_layers = num_layers
        self.seq_len = 5  # Industry token + 4 quarters

        # Input representation layer (financial + macro + industry)
        self.input_representation = InputRepresentationFullModel(
            financial_dim=156,
            macro_dim=36,
            num_industries=num_industries,
            output_dim=d_model
    ...
    def forward(self, batch_data):
        # 1. Input representation (financial + macro + industry)
        embeddings = self.input_representation(batch_data)
        batch_size, seq_len, d_model = embeddings.shape

        # 2. Generate FULL CONNECTION attention mask (no restrictions)
        attention_mask = FullConnectionAttentionMask.generate_mask(seq_len, embeddings.device)
        ...
        # 4. Use industry token representation for classification
        industry_representation = x[:, 0, :]  # Use industry token (index 0)
...
class AblationTrainer:
    ...
    def save_best_model(self, save_path, best_epoch):
        ...
        print(f"💾 Best full model (no structured mask) saved to: {save_path}_best_epoch_{best_epoch}.pth")
...
class AblationTester:
    ...
    def plot_confusion_matrix(self, targets, predictions):
        ...
        plt.title('Confusion Matrix - Full Model (No Structured Mask)', fontsize=16, fontweight='bold')
        plt.ylabel('True Label', fontsize=12)
        plt.xlabel('Predicted Label', fontsize=12)
        plt.tight_layout()
        plt.savefig('confusion_matrix_full_model_no_structured_mask.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    """Main function for full model without structured mask ablation study"""
    print("Starting Full Model (No Structured Mask) Ablation Study")
    print("Model: FullModelNoStructuredMask (Financial + Macro + Industry + Full Connection Mask)")
    ...
    # Create full model without structured mask
    model = FullModelNoStructuredMask(
        d_model=256,
        nhead=8,
        num_layers=4,
        ...
        num_industries=5  # Assuming 5 industry categories
    )

    # Parameter statistics
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nFull Model (No Structured Mask) parameter statistics:")
    print(f"  Total parameters: {total_params:,}")
    print(f"  Input: Financial data (156) + Macro data (36) + Industry information")
    print(f"  Feature fusion: Financial + Macro addition per quarter")
    print(f"  Sequence length: 5 (industry token + 4 quarters)")
    print(f"  Attention: FULL CONNECTION mask (no restrictions)")
    print(f"  Prediction: Industry token representation")
    ...
    os.makedirs('full_att_mask', exist_ok=True)
    save_path = 'full_att_mask'

    # Start training
    best_epoch, avg_epoch_time = trainer.train(num_epochs=250, save_path=save_path)

    print(f"\nFull model (no structured mask) ablation training completed! Best epoch: {best_epoch}")
    if avg_epoch_time is not None:
        print(f"Average training time per epoch: {avg_epoch_time:.2f} seconds")
    print(f"Best full model (no structured mask) saved to: {save_path}_best_epoch_{best_epoch}.pth")
    ...
    print(f"\n🎯 Full model (no structured mask) ablation study completed successfully!")
    print(f"📊 Performance report saved as: performance_full_model_no_structured_mask_*.txt")
    print(f"📈 Confusion matrix saved as: confusion_matrix_full_model_no_structured_mask.png")
